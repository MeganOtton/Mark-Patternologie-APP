import { isDrawing, drawingPath, allDrawings } from './drawingState.js';
import { drawGrid, gridDivisions } from './grid.js';
import { redrawCanvas} from './drawingCore.js';

console.log(`isDrawing: ${isDrawing}`);

export function tileDrawingHalfDrop(ctx: CanvasRenderingContext2D, canvas:HTMLCanvasElement, gridDivisions:number) {
    ...
    for (let col = 0; col < gridDivisions; col++) {
        ...
        for (let row = 0; row < gridDivisions; row++) {
            const cellX = col * cellSize;
                ...
                if (cellY < 0 || cellY >= canvas.height) continue;
